...
export function TimerPage() {
  ...
  const handleStop = useCallback(async () => {
    ...
    if (timerSeconds > 0) {
      ...
      toast(t("timer.sessionSaved"));
    }
  }, [timerSeconds, selectedProjectId, projects, settings, addSession, timerDescription, setTimerRunning, setTimerPaused, setTimerStartedAt, setTimerPausedElapsed, setTimerSeconds, setTimerDescription, toast]);
              ...
              return (
                <tr key={session.id}>
                  ...
                  <td className={s.actionsCell}>
                    ...
                    <button className={s.actionBtn} onClick={() => { deleteSession(session.id); toast(t("timer.sessionDeleted")); }}><Trash2 style={{ width: 14, height: 14 }} /></button>
                  </td>
                </tr>
              );
            })}
